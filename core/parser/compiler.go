...
type Compiler interface {
	Compile() *automata.FSM
}

//Compile a RegExpr into a Finite State Machine
func (r *RegExpr) Compile() *automata.FSM {
	if r.union != nil {
...
func (u *Union) Compile() *automata.FSM {
	a := u.simple.Compile()
	b := u.regex.Compile()
	a.Union(b)
	return a
...
func (s *SimpleExpr) Compile() *automata.FSM {
...
func (c *Concatenation) Compile() *automata.FSM {
	a := c.basic.Compile()
	b := c.simple.Compile()
	a.Append(b)
	return a
...
func (b *BasicExpr) Compile() *automata.FSM {
	if b.star != nil {
...
func (s *Star) Compile() *automata.FSM {
	a := s.element.Compile()
	a.Loop()
	return a
}
...
func (p *Plus) Compile() *automata.FSM {
	a := p.element.Compile()
	b := p.element.Compile()
	b.Loop()
	a.Append(b)
...
func (q *Question) Compile() *automata.FSM {
	a := q.element.Compile()
	a.Nodes[0] = true
	return a
}
...
func (e *Element) Compile() *automata.FSM {
	if e.group != nil {
...
func (p *PositiveSet) Compile() *automata.FSM {
	return p.items.Compile()
}

//Compile a NegativeSet into a Finite State Machine
func (n *NegativeSet) Compile() *automata.FSM {
	a := n.items.Compile()
	// a.Negate()
	return a
}
...
func (s *SetItems) Compile() *automata.FSM {
	a := s.item.Compile()
...
func (s *SetItem) Compile() *automata.FSM {
	if s.rnge != nil {
...
func (r *Range) Compile() *automata.FSM {
	a := automata.NewFSM(r.start.base.Value)
	for i := r.start.base.Value + 1; i <= r.end.base.Value; i++ {
		a.AddEdge(0, 1, i)
	}
	return a
}
...
func (c *Character) Compile() *automata.FSM {
	return c.base.Compile()
}

//Compile a Base into a Finite State Machine
func (b *Base) Compile() *automata.FSM {
	return automata.NewFSM(b.Value)
}
